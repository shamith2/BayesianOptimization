...
def BO(geometry=None, n_eval=200, path=None, func=None, ndim=None, boundary=False, ard=False, origin=False, warping=False, parallel=False):
	...
	if path is None:
		...
		if func.__name__ == 'stochastic_depth_resnet_cifar100':  # func_name changed to __name__
		...
		data_config_file = open(data_config_filename, 'rb')   # changed 'r' to 'rb'
		for key, value in pickle.load(data_config_file).items():  # changed iteritems() to items()
			if key != 'logfile_dir':
				exec(key + '=value')
	...
	if path is None:
		...
		f = open(data_config_filename, 'wb')  # changed 'w' to 'wb'
...
if __name__ == '__main__':
	...
	if args.func_name is not None:
		exec('func=' + args.func_name) # added parentheses
		args_dict['func'] = func
